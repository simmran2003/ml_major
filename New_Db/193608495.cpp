...
#line 2 "Library/C++/convolution/div-multi_zeta-mebius_transform.hpp"
...
template<typename T>
void divisor_mebius_transform(vector<T> &A){
    int n = A.size();
    for(int i = 1; i < n; i++){
        for(int j = 2 * i; j < n; j += i){
            A[j] -= A[i];
...
template<typename T>
void multiple_mebius_transform(vector<T> &A){
    int n = A.size();
    for(int i = n - 1; i > 0; i--){
        for(int j = 2 * i; j < n; j += i){
            A[i] -= A[j];
